...
MASTER_URI=http://192.168.8.7:11311
...
# tmux send-keys -t $SESSION_NAME "sws; sleep 4; roslaunch snavquad_interface voxl_vio.launch pub_odom_base_link:=true imu_rate:=150"
# tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sws; sleep 9; roslaunch snavquad_interface quad_control.launch use_vicon:=true" Enter
...
# tmux split-window -t $SESSION_NAME
# tmux send-keys -t $SESSION_NAME "sws; sleep 7; rosrun kr_trackers twist_to_velocity_goal.py __ns:=${MAV_NAME}"
# tmux split-window -t $SESSION_NAME
# tmux send-keys -t $SESSION_NAME "sws; roscd snavquad_interface/scripts/capture; chmod a+x record.sh; ./record.sh $MAV_ID"
# tmux split-window -t $SESSION_NAME
# tmux send-keys -t $SESSION_NAME "sws; sleep 12; roslaunch kr_xbee_ros kr_xbee_ros_voxl.launch"
# tmux select-layout -t $SESSION_NAME tiled

# tmux new-window -t $SESSION_NAME -n "Dock"
# tmux send-keys -t $SESSION_NAME "sws; sleep 12; roscd snavquad_interface/scripts; bash run_docker_tag.sh"
